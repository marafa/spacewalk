...
#this script will register this box with a spacewalk server
...
[ "$version" -eq "5" ] && rpm -Uvh http://dl.fedoraproject.org/pub/epel/$version/$machine/python-hashlib-20081119-4.el$version.$machine.rpm
...
#wget http://spacewalk/pub/up2date -O /etc/sysconfig/rhn/up2date
